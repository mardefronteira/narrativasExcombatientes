class ScenePlayer {
  ...
  display() {
    ...
    let target = data[`${this.type}s`].find(
      (target) => target["id"] === this.id
    );
    ...
    if (this.id !== "E-01") {
      ...
      document.getElementById(`scene-button`).addEventListener("click", (e) => {
        this.playPause(e);
      });
      ...
      this.timecodes = data["audios"].filter(
        (relation) => relation["scene"] === this.id
      );
      ...
      name.innerHTML = data["scenes"].filter(
        (scene) => scene["id"] === this.id
      )[0].scene;
      ...
      document.getElementById(
        `${this.id}-time`
      ).innerHTML = `– 00:00 / ${this.scene.duration}`;

      // set background color
      document
        .getElementById("player-background")
        .setAttribute("fill", this.playerColor);
      ...
      this.frame = data["frames"].find((frame) =>
        frame.scenes.includes(this.id)
      );
      if (this.frame.id === "M-06") {
        document
          .querySelector(`#${this.id}-guide-button`)
      ...
      document.querySelector("#E-01-button").addEventListener("click", () => {
        openGuide("guide-01");
      });
    }
  }
  ...
  playPause(e) {
    ...
    document
      .getElementById(`scene-${buttonToShow}-button`)
      .classList.remove("hidden");
    document
      .getElementById(`scene-${buttonToHide}-button`)
  ...
  updatePlayer(target) {
    if (this.hasBegun) {
      ...
      let currentCharacter = this.timecodes.find(
        (slice) => slice.start_sec < currentTime && slice.end_sec > currentTime
      );
      if (currentCharacter) {
